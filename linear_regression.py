...
for i in range(len(X1)):
    ...
    results[i] = [theta1[i], theta2[i],np.amin(Y_vals), np.amax(Y_vals), RMSE_test, RMSE_train, MAE_test, MAE_train, R2_test, R2_train]


results_df = pd.DataFrame(results)
results_df.columns = ['Theta of Bob 1', 'Theta of Bob 2', 'Min of X1', 'Max of X1', "RMSE of test", "RMSE of train", "MAE of test", "MAE of train", "R2 of test", "R2 of train"]
results_df.to_csv('Linear Regression Without Poly Feature Map.csv',index=False)
print(results_df)